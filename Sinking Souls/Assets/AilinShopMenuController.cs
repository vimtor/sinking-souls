...
public class AilinShopMenuController : MonoBehaviour {
    ...
    void Update()
    {
        ...
        if (itemArr == null || itemArr.Length == 0)
        {
            ...
            foreach (Button but in itemArr)
            {
                ...
                int price = 0;
                int.TryParse(but.gameObject.transform.GetChild(3).GetComponent<TextMeshProUGUI>().text, out price);
    ...
    void Highlight(Button item)
    {
        ...
        if (item.gameObject.transform.GetChild(2).GetComponent<TextMeshProUGUI>().color == normalTextColor) SetItemColor(item, highlightedTextColor, highlightedImageColor);
        else SetItemColor(item, highlightedUnavailableTextColor, highlightedUnavailableImageColor);
    }

    void Normalize(Button item)
    {
        if (item.gameObject.transform.GetChild(2).GetComponent<TextMeshProUGUI>().color == highlightedTextColor) SetItemColor(item, normalTextColor, normalImageColor);
