...
public class selectionTriangleBehaviour : MonoBehaviour {
	...
	void Update () {
        ...
        if (GameController.instance.player.GetComponent<Player>().lockedEnemy != null) {
            transform.position = GameController.instance.player.GetComponent<Player>().lockedEnemy.transform.position + Vector3.up * (height + Mathf.Sin(Time.time * floatingSpeed) * floatingAmount);
